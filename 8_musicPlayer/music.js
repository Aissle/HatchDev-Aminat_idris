...
class MusicPlayer {
    constructor(title, Ivalue) {
        ...
        if (Ivalue == undefined) {
            ...
            this.firstSong = head;
            console.log(`${this.title} music player created, ${head} added to playlist`);
        }
    }
    addSong(song) {
        const newNode = new SongNode(song);
        if (!this.firstSong)
            this.firstSong = newNode;
        else {
    ...
    shuffle() {
        ...
        // Step 1: Collect songs into an array
        const nodes = [];
        let currentNode = this.firstSong;
        while (currentNode !== null) {
        ...
        // Step 2: Shuffle the array using Math.random()
        for (let i = nodes.length - 1; i > 0; i--) {
            const j = Math.floor(Math.random() * (i + 1));
            [nodes[i], nodes[j]] = [nodes[j], nodes[i]];
        }
        // Step 3: Reconstruct linked list from shuffled nodes
